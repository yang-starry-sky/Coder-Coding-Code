...
typedef vector<pair<double,int> > jumpingPoint;
...
void solution(){
    init();
    double weight[MAX_NUM]={2.1,2.2,6.3,5.4,4.5};
    int value[MAX_NUM]={6,3,5,4,6};
    jumpingPoint v[MAX_NUM];
    v[0].push_back(make_pair(0.0,0));
    ...
    for(int i=1;i<MAX_NUM;i++){
        ...
        int k=0;
        int len2=q.size();
        v[i].push_back(make_pair(0.0,0));
        printf("i=%d\n",i);
        printf("P:");
        for(int a=0;a<len1;a++)
            if(a!=len1-1)
                printf("%.2f %d, ",p[a].first,p[a].second);
            else
                printf("%.2f %d\n",p[a].first,p[a].second);
        printf("Q:");
        for(int a=0;a<len2;a++)
            if(a!=len2-1)
                printf("%.2f %d, ",q[a].first,q[a].second);
            else
    ...
    for(int i=0;i<MAX_NUM;i++){
        ...
        for(int j=0;j<len;j++)
            if(j!=len-1)
                printf("%.2f %2d, ",v[i][j].first,v[i][j].second);
            else
